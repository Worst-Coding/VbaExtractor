...
The output `.txt` file keeps the full original filename (including the `.swp` extension) and simply appends `.txt` — e.g. `Template_Macro.swp` becomes `Template_Macro.swp.txt`.
...
Exporting a `.swp` file (e.g. `Template_Macro.swp`) produces a `.txt` file named `Template_Macro.swp.txt`, which looks roughly like this:
 
```
--- VBA export from: Template_Macro.swp ---
...
```
2026-07-17_14-32_Template_Macro.swp/
└── Module1.bas
```
## License
